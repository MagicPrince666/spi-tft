...
_listbox_list * list_search(_listbox_list *listx,uint16_t index)
{
	...
	while(listtemp->prevlist!=NULL)listtemp = (_listbox_list *)listtemp->prevlist;//追溯到最开始的list
	while(1)//寻找编号为index的list
	{
		if(index==icnt)break;	 
		if(listtemp->nextlist!=NULL)
		{
			listtemp = (_listbox_list *)listtemp->nextlist;    
			icnt++;//计数增加
...
uint8_t listbox_addlist(_listbox_obj * listbox,uint8_t *name)
{
	...
	}else //已经创建了
	{
		...
		while(listtemp->nextlist!=NULL)listtemp = (_listbox_list *)listtemp->nextlist;	 
...
void listbox_draw_list(_listbox_obj *listbox)
{
	...
	{	
		...
		listx = (_listbox_list *)listx->nextlist;
 	}
}
//画listbox
//listbox:listbox指针
