...
_filelistbox_list * filelist_search(_filelistbox_list *filelistx,uint16_t index)
{
	...
	while(filelisttemp->prevlist != NULL)filelisttemp = (_filelistbox_list *)filelisttemp->prevlist;//追溯到最开始的list
	while(1)//寻找编号为index的list
	{
		if(index==icnt)break;	 
		if(filelisttemp->nextlist!=NULL)
		{
			filelisttemp = (_filelistbox_list *)filelisttemp->nextlist;    
			icnt++;//计数增加
...
uint8_t filelistbox_addlist(_filelistbox_obj * filelistbox,uint8_t *name,uint8_t type)
{
	...
	filelistx->name = (uint8_t*)gui_memex_malloc(strlen((const char*)name)+1);	//分配内存来存放name,外加一个结束符.
	...
	}else //已经创建了
	{
		...
		while(filelisttemp->nextlist!=NULL)filelisttemp = (_filelistbox_list *)filelisttemp->nextlist;//偏移到最后一个list	 
...
void filelistbox_draw_list(_filelistbox_obj *filelistbox)
{
	...
	{	
		...
		filelistx = (_filelistbox_list *)filelistx->nextlist;
 	}
}
//显示filelistbox的路径 
//filelistbox:filelistbox指针
	...
	if(plen)//需要显示
	{
		...
		if(plen*(gui_phy.listfsize/2)>(filelistbox->width-filelistbox->icosize-7))
		{
			...
			while(plen*(gui_phy.listfsize/2)>(filelistbox->width-offset-filelistbox->icosize-7))
			{
 			...
 			gui_show_ptstr(filelistbox->left+filelistbox->icosize+7,filelistbox->top+(gui_phy.listheight-gui_phy.listfsize)/2,filelistbox->width+filelistbox->left,gui_phy.lcdheight,0,filelistbox->lncolor,gui_phy.listfsize,(uint8_t *)"...",1);//显示3点
		}   	
		gui_show_ptstr(filelistbox->left+offset+filelistbox->icosize+7,filelistbox->top+(gui_phy.listheight-gui_phy.listfsize)/2,filelistbox->width+filelistbox->left,gui_phy.lcdheight,0,filelistbox->lncolor,gui_phy.listfsize,(uint8_t *)path,1);//显示路径
	}
}	 

//磁盘容量字符串
const char* FLBOX_DISK_SIZE_CAPTION_TBL[GUI_LANGUAGE_NUM]=
{
	"磁盘容量:","磁盤容量:","Capacity:"	
};
//磁盘可用空间字符串
const char* FLBOX_DISK_FREE_CAPTION_TBL[GUI_LANGUAGE_NUM]=
{
	"可用空间:","可用空間:","Free:"	
};
//文件夹字符串
const char* FLBOX_FOLDER_CAPTION_TBL[GUI_LANGUAGE_NUM]=
{
	"文件夹:","文件夾:","Folder:"	
};
//磁盘可用空间字符串
const char* FLBOX_FILES_CAPTION_TBL[GUI_LANGUAGE_NUM]=
{
	"  文件:","  文件:","  Files:"
}; 
//磁盘名字表
const char* FLIST_DISK_NAME_TBL[3][GUI_LANGUAGE_NUM]=
{
	{"SD卡","SD卡","SD Card",},
...
uint8_t FLIST_DISK_ORDER_TBL[3];
//filelist磁盘路径表
const char* FLIST_DISK_PATH_TBL[3]={"0:","1:","2:"};
...
void filelistbox_show_info(_filelistbox_obj *filelistbox)
{
	...
	uint8_t * temp1;
	uint8_t * mark;
	temp = (uint8_t*)gui_memin_malloc(60);	//用于存放字符串
	if(temp==NULL)return;
 	temp1 = (uint8_t*)gui_memin_malloc(30);	//用于存放数字串
	if(temp1==NULL)
	{
		gui_memin_free(temp);	
		return;  
	...
	if(filelistx->type==FICO_DISK)//当前选中的是磁盘
	{
		...
		//res = f_getfree((const char*)mark,(DWORD*)&freclust,&fs1);//获得剩余容量
	    if(res == 0)
		{											   
...
uint8_t filelistbox_scan_filelist(_filelistbox_obj *filelistbox)
{
	...
	{
		flboxdir = opendir((const char*)filelistbox->path); //打开一个目录
		if(flboxdir != NULL)		//打开目录成功了.
		{
		...
		while(res==0)
		{
			...
			if(res!=0||flboxfinfo.fname[0]==0)
			{
				if(mark==0)	//文件夹查找完了
				{
					mark=1;	//标记查找目标文件
					fclosedir(&flboxdir);
					flboxdir = f_opendir((const char*)filelistbox->path); //再次打开该目录
 					continue;
				}
				break;		//错误了/到末尾了,退出   
			}
		...
		{
			fn = (uint8_t*)gui_memin_realloc(filelistbox->findextbl,filelistbox->filecnt*2);//释放多余的内存
...
void filelistbox_add_disk(_filelistbox_obj *filelistbox)
{
	...
	if(filelistbox->memdevflag&(1<<0)) 	//SD卡在位
	{
		...
		//res=f_getlabel ((char *)"0:",(char *)tbuf,(DWORD*)&sn);
		if(res==0&&strlen((const char *)tbuf))//读取成功，且磁盘名字不为空。
		{
	...
	if(filelistbox->memdevflag&(1<<1)) 	//FLASH盘在位
	{
		...
		//res=f_getlabel ((char *)"1:",(char *)tbuf,(DWORD*)&sn);
		if(res==0&&strlen((const char *)tbuf))//读取成功，且磁盘名字不为空。
		{
			filelistbox_addlist(filelistbox,tbuf,0);//添加一个磁盘  
		}else //读取失败/磁盘盘符为空的时候，采用默认名字 
	...
	if(filelistbox->memdevflag&(1<<2)) 	//U盘在位
	{
		...
		//res=f_getlabel ((char *)"2:",(char *)tbuf,(DWORD*)&sn);
		if(res==0&&strlen((const char *)tbuf))//读取成功，且磁盘名字不为空。
		{
			filelistbox_addlist(filelistbox,tbuf,0);//添加一个磁盘  
		}else //读取失败/磁盘盘符为空的时候，采用默认名字 
...
void filelistbox_check_filelist(_filelistbox_obj *filelistbox)
{
	...
	{   
		...
		{	    
 			filelistbox_addlist(filelistbox,(uint8_t*)"..",1);	//添加返回上一层伪文件夹 
		}
		filelistbox_draw_listbox(filelistbox);	//重画
 	}else 
	{
