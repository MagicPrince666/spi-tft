...
////////////////////////////GUI通用字符串集//////////////////////////////
//确认
const char* GUI_OK_CAPTION_TBL[GUI_LANGUAGE_NUM]=
{
	"确定","確定","OK"
};
//选项
const char* GUI_OPTION_CAPTION_TBL[GUI_LANGUAGE_NUM]=
{
	"选项","選項","MENU"
};
//返回
const char* GUI_BACK_CAPTION_TBL[GUI_LANGUAGE_NUM]=
{
	"返回","返回","BACK"
};
//取消 
const char* GUI_CANCEL_CAPTION_TBL[GUI_LANGUAGE_NUM]=
{
	"取消","取消","CANCEL"
};
//退出
const char* GUI_QUIT_CAPTION_TBL[GUI_LANGUAGE_NUM]=
{
	"退出","退出","QUIT"	    
};
///////////////////////////////////////////////////////////////////////////
...
{       
	...
	uint8_t* pico;
	icosize=size*size;
	colorbuf = (uint16_t*)gui_memin_malloc(2*icosize);//申请内存
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				if(mode&0X80)gui_show_pchar(x+(size/2)*t,y,gui_phy.lcdwidth,gui_phy.lcdheight,0,color,size,'0',mode&0xf);//填充0
				else gui_show_pchar(x+(size/2)*t,y,gui_phy.lcdwidth,gui_phy.lcdheight,0,color,size,' ',mode&0xf);
 				continue;
			}else enshow=1; 
		 	 
		}
		gui_show_pchar(x+(size/2)*t,y,gui_phy.lcdwidth,gui_phy.lcdheight,0,color,size,temp+'0',mode&0xf);
	}
}
//将数字转为字符串
//str:字符串存放地址.
    ...
    { 
        if(!bHz)
        {
	        ...
	        {      	   
				...
				if(EnDisp==1)	//使能显示
				{
	 				gui_show_pchar(x,y,xend,yend,offset,color,size,*str,mode);//显示一个字符	 
...
void gui_show_string(uint8_t*str,uint16_t x,uint16_t y,uint16_t width,uint16_t height,uint8_t font,uint16_t fcolor)
{
	...
	{		
		...
		}else//是字符
		{
			...
			gui_show_pchar(xpos,ypos,endx,endy,0,fcolor,font,*str,1);		   
			xpos+=font/2;
			str+=1;	 			
		}
