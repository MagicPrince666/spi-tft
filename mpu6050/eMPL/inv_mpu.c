 ...
 * iic_read(unsigned char slave_addr, unsigned char reg_addr,
...
#define iic_write   MPU_Write_Len
#define iic_read    MPU_Read_Len
...
#define iic_read    msp430_iic_read
...
#define iic_read(a, b, c, d)    twi_read(a, b, d, c)
...
int mpu_reg_dump(void)
{
    ...
    for (ii = 0; ii < st.hw->num_reg; ii++) {
        if (ii == st.reg->fifo_r_w || ii == st.reg->mem_r_w)
            continue;
        if (iic_read(st.hw->addr, ii, 1, &data))
            return -1;
        log_i("%#5x: %#5x\r\n", ii, data);
    }
    return 0;
...
int mpu_read_reg(unsigned char reg, unsigned char *data)
{
    ...
    if (reg >= st.hw->num_reg)
        return -1;
    return iic_read(st.hw->addr, reg, 1, data);
}
...
int mpu_init(void)
{
    ...
    if (iic_read(st.hw->addr, st.reg->accel_offs, 6, data))
        return -1;
    ...
    if (rev) {
        ...
        if (iic_read(st.hw->addr, st.reg->prod_id, 1, data))
            return -1;
        rev = data[0] & 0x0F;
        if (!rev) {
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
    ...
    if (iic_read(st.hw->addr, st.reg->raw_gyro, 6, tmp))
        return -1;
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
    ...
    if (iic_read(st.hw->addr, st.reg->raw_accel, 6, tmp))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
    data[1] = (tmp[2] << 8) | tmp[3];
    data[2] = (tmp[4] << 8) | tmp[5];
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
    ...
    if (iic_read(st.hw->addr, st.reg->temp, 2, tmp))
        return -1;
    raw = (tmp[0] << 8) | tmp[1];
    if (timestamp)
        get_ms(timestamp);
...
int mpu_set_accel_bias(const long *accel_bias)
{
    ...
    if (iic_read(st.hw->addr, 3, 3, data))
        return -1;
    fg[0] = ((data[0] >> 4) + 8) & 0xf;
    fg[1] = ((data[1] >> 4) + 8) & 0xf;
    fg[2] = ((data[2] >> 4) + 8) & 0xf;
    ...
    if (iic_read(st.hw->addr, 0x06, 6, data))
        return -1;
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors & INV_XYZ_COMPASS)
        ...
        mpu_set_bypass(0);
#else
    if (iic_read(st.hw->addr, st.reg->user_ctrl, 1, &user_ctrl))
        return -1;
...
int mpu_get_int_status(short *status)
{
    ...
    if (!st.chip_cfg.sensors)
        return -1;
    if (iic_read(st.hw->addr, st.reg->dmp_int_status, 2, tmp))
        return -1;
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    if (iic_read(st.hw->addr, st.reg->fifo_count_h, 2, data))
        return -1;
...
//    log_i("FIFO count: %hd\n", fifo_count);
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (iic_read(st.hw->addr, st.reg->int_status, 1, data))
            return -1;
    ...
    if (iic_read(st.hw->addr, st.reg->fifo_r_w, packet_size, data))
        return -1;
    ...
    unsigned char *more)
{
    ...
    if (iic_read(st.hw->addr, st.reg->fifo_count_h, 2, tmp))
        return -1;
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        if (iic_read(st.hw->addr, st.reg->int_status, 1, tmp))
            return -1;
    ...
    if (iic_read(st.hw->addr, st.reg->fifo_r_w, length, data))
        return -1;
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (iic_read(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        tmp &= ~BIT_AUX_IF_EN;
...
static int get_accel_prod_shift(float *st_shift)
{
    ...
    if (iic_read(st.hw->addr, 0x0D, 4, tmp))
        return 0x07;
...
static int gyro_self_test(long *bias_regular, long *bias_st)
{
    ...
    if (iic_read(st.hw->addr, 0x0D, 3, tmp))
        return 0x07;
...
static int compass_self_test(void)
{
    ...
    do {
        delay_ms(10);
        if (iic_read(st.chip_cfg.compass_addr, AKM_REG_ST1, 1, tmp))
            goto AKM_restore;
    ...
    if (iic_read(st.chip_cfg.compass_addr, AKM_REG_HXL, 6, tmp))
        goto AKM_restore;
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        if (iic_read(st.hw->addr, st.reg->fifo_r_w, MAX_PACKET_LENGTH, data))
            return -1;
        ...
        unsigned char *data)
{
    ...
    if (iic_write(st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (iic_read(st.hw->addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
...
int setup_compass(void)
{
    ...
    for (akm_addr = 0x0C; akm_addr <= 0x0F; akm_addr++) {
        int result;
        result = iic_read(akm_addr, AKM_REG_WHOAMI, 1, data);
    ...
    if (iic_read(st.chip_cfg.compass_addr, AKM_REG_ASAX, 3, data))
        return -1;
...
#ifdef AK89xx_BYPASS
    if (iic_read(st.chip_cfg.compass_addr, AKM_REG_ST1, 8, tmp))
        return -1;
    tmp[8] = AKM_SINGLE_MEASUREMENT;
    if (iic_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp+8))
        return -1;
#else
    if (iic_read(st.hw->addr, st.reg->raw_compass, 8, tmp))
        return -1;
#endif
