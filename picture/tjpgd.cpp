...
)
{
	...
	while (ndata) {	/* Process all tables in the segment */
		...
		pb = (unsigned char*)alloc_pool(jd, 16);			/* Allocate a memory block for the bit distribution table */
		if (!pb) return JDR_MEM1;			/* Err: not enough memory */
		...
		ph = (short unsigned int*)alloc_pool(jd, np * sizeof (WORD));/* Allocate a memory block for the code word table */
		...
		ndata -= np;
		pd = (unsigned char*)alloc_pool(jd, np);			/* Allocate a memory block for the decoded data */
...
)
{
	...
	jd->inbuf = seg = (unsigned char*)alloc_pool(jd, JD_SZBUF);		/* Allocate stream input buffer */
	...
	for (;;) {
		...
		case 0xDA:	/* SOS */
			...
			jd->mcubuf = (unsigned char*)alloc_pool(jd, (n + 2) * 64);	/* Allocate MCU working buffer */
...
{ 
    uint32_t  rb; //读取到的字节数
    FILE *dev = (FILE*)jd->device;  //待解码的文件的信息，使用FATFS中的FILE结构类型进行定义
    if(buf)     				//读取数据有效，开始读取数据
    { 
        rb = fread(buf,sizeof(uint8_t),num,dev);//调用FATFS的fread函数，用于把jpeg文件的数据读取出来
        return rb;        		//返回读取到的字节数目
    }else 
	{
		int ret = fseek(dev,ftell(dev) + num ,SEEK_SET);
		return (ret == 0)?num:0;
	//(fseek(dev,ftell(dev)+num ,SEEK_END) == 0)?num:0;//重新定位数据点，相当于删除之前的n字节数据 
	}
}  
//采用填充的方式进行图片解码显示
//jd:储存待解码的对象信息的结构体
...
{ 
	...
	uint16_t *pencolor=(uint16_t*)rgbbuf;
	uint16_t width=rect->right-rect->left+1;	//图片的宽度 16
	uint16_t height=rect->bottom-rect->top+1;	//图片的高度 16

	uint16_t posx = 0,posy = 0,xwidth = 0;
   	for(i=0;i<height;i++)//y坐标
	{
		...
		for(j=0;j<width;j++)//x坐标
		{
			...
			SPI_LCD_RAM[xwidth * posy + posx] = *pencolor;//满足条件 加入缓存
			posx ++;
	...
	pic_phy.fillcolor(picinfo.S_XOFF + realx+1 - posx,picinfo.S_YOFF + realy+1 - posy ,posx,posy,SPI_LCD_RAM);
    return 0;    //返回0,使得解码工作继续执行 
} 
//解码jpeg/jpg文件s
...
{  
	...
	{
		//得到JPEG/JPG图片的开始信息		 
		f_jpeg = fopen((const char*)filename,"r");//打开文件
		...
		{ 
			...
			{ 	
				...
				//printf("picinfo.ImgWidth = %d picinfo.ImgHeight = %d\n",picinfo.ImgWidth,picinfo.ImgHeight);
