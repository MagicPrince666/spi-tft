...
uint8_t stdbmp_decode(const uint8_t *filename) 
{
	...
	uint16_t width = picinfo.S_Width - picinfo.S_XOFF;//一行宽度
	...
	databuf=bmpreadbuf;
#endif
	f_bmp = fopen((const char*)filename,"r");//打开文件	 						  
	if(f_bmp == NULL) return 1;
	else//打开成功.
	{ 
		br = fread(databuf,1,readlen,f_bmp);	//读出readlen个字节  
	...
	//	printf("start ai_draw_init\n");			
		...
		{				 
			...
			if(br == 0)//读取完毕 开始显示
			{
				pic_phy.fillcolor(picinfo.S_XOFF,picinfo.S_YOFF,picinfo.S_Width - picinfo.S_XOFF,picinfo.S_Height - picinfo.S_YOFF,SPI_LCD_RAM);
				break;		
			}
			bmpbuf = databuf;
	 	 	count = 0;
...
//	uint16_t width = picinfo.S_Width - picinfo.S_XOFF;//一行宽度
	...
	if(f_bmp != NULL)//打开成功.
	{ 
		br = fread(databuf,sizeof(BITMAPINFO),readlen,f_bmp);//读出BITMAPINFO信息 
		pbmp=(BITMAPINFO*)databuf;					//得到BMP的头部信息   
		color_byte=pbmp->bmiHeader.biBitCount/8;	//彩色位 16/24/32  
		biCompression=pbmp->bmiHeader.biCompression;//压缩方式
		picinfo.ImgHeight=pbmp->bmiHeader.biHeight;	//得到图片高度
		...
		{  							   
			...
			fseek(f_bmp,pbmp->bmfHeader.bfOffBits,SEEK_SET);	//偏移到数据起始位置 	  
			while(1)
			{	     
				br = fread(databuf,1,readlen,f_bmp);	//读出readlen个字节
				if(br <= 0)printf("fread error!\n");
			...
			pic_phy.fillcolor(picinfo.S_XOFF,picinfo.S_YOFF,picinfo.S_Width,picinfo.S_Height,SPI_LCD_RAM); 
		}	
		fclose(f_bmp);//关闭文件      
...
{				
...
#if BMP_USE_MALLOC == 1	//使用malloc	
	databuf=(uint16_t*)malloc(1024);		//开辟至少bi4width大小的字节的内存区域 ,对240宽的屏,480个字节就够了.
	if(databuf==NULL)return PIC_MEM_ERR;		//内存申请失败.
	f_bmp=(FILE *)malloc(sizeof(FILE));	//开辟FILE字节的内存区域 
	...
	if(mode==1)f_bmp = fopen((const char*)filename,"rw");//尝试打开之前的文件
 	if(mode==0||res==0x04)f_bmp = fopen((const char*)filename,"w+");//模式0,或者尝试打开失败,则创建新文件		   
 	if((hbmp.bmiHeader.biWidth*2)%4)//水平像素(字节)不为4的倍数
	{
 	...
 	if(f_bmp != NULL)//创建成功
	{
		res=fwrite((uint8_t*)&hbmp,1,bmpheadsize,f_bmp);//写入BMP首部  
		for(ty=y+height-1;hbmp.bmiHeader.biHeight;ty--)
		{
			pixcnt=0;
 			for(tx=x;pixcnt!=(bi4width/2);)
			{
				if(pixcnt<hbmp.bmiHeader.biWidth)databuf[pixcnt] = POINT_COLOR;//LCD_ReadPoint(tx,ty);//读取坐标点的值 
				else databuf[pixcnt]=0Xffff;//补充白色的像素.  
				pixcnt++;
				tx++;
			}
			hbmp.bmiHeader.biHeight--;
			res=fwrite((uint8_t*)databuf,1,bi4width,f_bmp);//写入数据
		}
		fclose(f_bmp);
	}	    
#if BMP_USE_MALLOC == 1	//使用malloc	
