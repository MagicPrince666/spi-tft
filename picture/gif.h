...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
uint8_t gif_check_head(FILE *file);														    //检测GIF头
uint16_t gif_getrgb565(uint8_t *ctb);																//将RGB888转为RGB565
uint8_t gif_readcolortbl(FILE *file,gif89a * gif,uint16_t num);									//读取颜色表
uint8_t gif_getinfo(FILE *file,gif89a * gif);												    //得到逻辑屏幕描述,图像尺寸等
...
uint16_t gif_getdatablock(FILE *gfile,uint8_t *buf,uint16_t maxnum);								   	//读取一个数据块
uint8_t gif_readextension(FILE *gfile,gif89a* gif, int *pTransIndex,uint8_t *pDisposal);		   	//读取扩展部分
int gif_getnextcode(FILE *gfile,gif89a* gif);										   	//从LZW缓存中得到下一个LZW码,每个码包含12位
int gif_getnextbyte(FILE *gfile,gif89a* gif);											//得到LZW的下一个码
uint8_t gif_dispimage(FILE *gfile,gif89a* gif,uint16_t x0,uint16_t y0,int Transparency, uint8_t Disposal);	//显示图片
void gif_clear2bkcolor(uint16_t x,uint16_t y,gif89a* gif,ImageScreenDescriptor pimge);		   	//恢复成背景色
uint8_t gif_drawimage(FILE *gfile,gif89a* gif,uint16_t x0,uint16_t y0);									//画GIF图像的一帧
